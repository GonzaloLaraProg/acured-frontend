...
export const ProfessionalSelectionModal = ({ 
  isOpen, 
  onClose, 
  onBack, 
  onContinue 
}: ProfessionalSelectionModalProps): JSX.Element | null => {
  if (!isOpen) return null;

  // Progress steps data
  const progressSteps = [
    { label: "Selección Servicio", active: false },
    { label: "Información paciente", active: false },
    { label: "Selección Profesional", active: true },
    ...
    >
      <div className="max-w-[572px] w-full max-h-[90vh] overflow-y-auto">
        <div className="flex flex-col gap-6 p-8 bg-neutralswhite rounded-lg border border-solid border-[#f1f4f2] shadow-shadow-base">
            ...
            {progressSteps.map((step, index) => (
              ...
              >
                <div className="inline-flex items-center gap-4">
                  ...
                  {index < progressSteps.length - 1 && <ChevronRightIcon className="h-4 w-4" />}
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Main Content */}
          <div className="flex flex-col items-start gap-6 pt-0 pb-6 w-full border-b border-[#dcdce2]">
            ...
            <div className="flex flex-col w-full items-start gap-3">
              ...
              <div className="flex flex-col items-start w-full border border-gray-200 rounded-lg overflow-hidden">
                {professionals.map((professional, index) => (
                  <div
                    key={index}
                    className={`bg-[#ffffff] flex items-center justify-between px-4 py-3 w-full hover:bg-gray-50 transition-colors ${
          ...
          <div className="flex items-start justify-between w-full">
            <Button
              variant="outline"
              className="flex-col justify-center bg-neutralswhite shadow-shadow-xs inline-flex items-center px-4 py-2 rounded-3xl border-primary-200 hover:bg-primary-50 hover:border-primary-300 transition-all duration-200"
              onClick={onBack}
            >
              <span className="font-text-text-xs-text-xs-font-medium font-[number:var(--text-text-xs-text-xs-font-medium-font-weight)] text-primary-800 text-[length:var(--text-text-xs-text-xs-font-medium-font-size)] text-right tracking-[var(--text-text-xs-text-xs-font-medium-letter-spacing)] leading-[var(--text-text-xs-text-xs-font-medium-line-height)] whitespace-nowrap [font-style:var(--text-text-xs-text-xs-font-medium-font-style)]">
                Atrás
            ...
            <Button 
              className="flex-col justify-center bg-primary-800 shadow-shadow-md inline-flex items-center px-4 py-2 rounded-3xl hover:bg-primary-900 transition-all duration-200"
              onClick={onContinue}
            >
